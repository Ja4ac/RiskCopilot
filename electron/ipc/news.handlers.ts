...
export function registerNewsHandlers(): void {
  // ---- News queries ----
  createIpcHandler(IPC_CHANNELS.NEWS_GET_LIST, async (_event, filters?: IpcGetNewsPayload) => {
    return newsService.getNews(filters)
  })

  createIpcHandler(IPC_CHANNELS.NEWS_GET_ASSET_NEWS, async (_event, assetId: string) => {
    return newsService.getAssetNews(assetId)
  })

  // ---- News Data Source CRUD ----
  createIpcHandler(IPC_CHANNELS.NEWS_LIST_SOURCES, async () => {
    const db = getDb()
  ...
  createIpcHandler(IPC_CHANNELS.NEWS_SAVE_SOURCE, async (_event, payload: { id?: string; name: string; base_url: string; is_enabled?: boolean }) => {
    const db = getDb()
  ...
  createIpcHandler(IPC_CHANNELS.NEWS_DELETE_SOURCE, async (_event, id: string) => {
    const db = getDb()
  ...
  createIpcHandler(IPC_CHANNELS.NEWS_SET_ACTIVE_SOURCE, async (_event, id: string) => {
    const db = getDb()
    db.prepare(`
  ...
  createIpcHandler(IPC_CHANNELS.NEWS_SYNC, async () => {
    ...
    return { inserted, provider: provider.name }
  })
}
