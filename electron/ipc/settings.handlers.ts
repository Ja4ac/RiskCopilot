...
export function registerSettingsHandlers(): void {
  ...
  ipcMain.handle('settings:readFile', async (_event, filePath: string) => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.SETTINGS_TEST_DATA_SOURCE, async (_event, payload: IpcTestDataSourcePayload) => {
    try {
      if (payload.source_type === 'market') {
        if (payload.source_id) {
          ...
          if (registry.has(payload.source_id)) {
            ...
            return { success: true, data: { healthy, provider: provider.name } }
          }
          ...
          const healthy = await provider.healthCheck()
          return { success: true, data: { healthy, provider: source.name } }
        } else if (payload.base_url) {
          const provider = new HttpMarketProvider('test', 'Test', payload.base_url)
          const healthy = await provider.healthCheck()
          return { success: true, data: { healthy, provider: 'Test' } }
        }
        return { success: false, error: '请提供 source_id 或 base_url' }
      }
      ...
      return { success: false, error: '暂不支持该数据源类型的测试' }
    } catch (error: any) {
      return { success: false, error: error.message }
    }
  })
  ...
  ipcMain.handle('settings:restoreBackup', async (_event, payload: { content: string }) => {
    try {
      ...
      portfolioService.recalculateAllPositions()
